...
function CheckoutCart() {
  ...
  return (
    ...
    { countriesInCart.length <= 0 && <div> 
      <h4>No Country in cart</h4></div> }
      <Link to ="/" > <Icon name='angle double left' size='huge' textAlign='center'/></Link>
    {countriesInCart.map(p =>(
         <List divided key ={p.id} className="content" >
          <List.Item className="content__items" >
            <List.Content floated='right'>
              <Button onClick ={() => dispatch(removeCountry(p))}>
                Remove
              </Button>
            </List.Content>
